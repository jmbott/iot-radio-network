...
void readAmp(uint8_t id) {
  byte data[8] = {0x00, 0x03, 0x00, 0x09, 0x00, 0x01, 0x55, 0xBF};
  Serial.print("--ReadAmp-- Address : ");
  Serial.print(id);
  Serial.print(" -- ");
  byte *p = (byte *)&id;
  data[0] = p[0];
  if (meterConn.write(data, 8) == 7) {
    meter_should_receive = 13;
    Serial.print("--Read Amp function finished, Address : "); Serial.println(id);
    return 1;
  }
  else {
    Serial.print("--Read Amp function finished, Address : "); Serial.println(id);
